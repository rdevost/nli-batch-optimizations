...
class ESIM(nn.Module):
    def __init__(self, embedding_dim: int, dim_hidden: int, batch_size: int, vocab_size: int,
        ...
        if torch.cuda.is_available():
            ...
            self.LSTM_encoder = nn.LSTM(input_size=embedding_dim,
                                        hidden_size=dim_hidden, batch_first=False).cuda()
            self.LSTM_encoder_rev = nn.LSTM(input_size=embedding_dim,
                                            hidden_size=dim_hidden, batch_first=False).cuda()
            self.LSTM_decoder = nn.LSTM(input_size=dim_hidden,
                                        hidden_size=dim_hidden, batch_first=False).cuda()
            self.LSTM_decoder_rev = nn.LSTM(input_size=dim_hidden,
                                            hidden_size=dim_hidden, batch_first=False).cuda()
        else:
            ...
            self.LSTM_encoder = nn.LSTM(input_size=embedding_dim,
                                        hidden_size=dim_hidden, batch_first=False)
            self.LSTM_encoder_rev = nn.LSTM(input_size=embedding_dim,
                                            hidden_size=dim_hidden, batch_first=False)
            self.LSTM_decoder = nn.LSTM(input_size=dim_hidden,
                                        hidden_size=dim_hidden, batch_first=False)
    ...
    def forward(self, x1, x1_mask, x2, x2_mask,l):
        ...
        for i in range(n_timesteps_premise):
            ...
            for j in range(n_timesteps_hypothesis):
                score_ij = torch.max(torch.mul(premise_list[i], hypothesis_list[j]), 1,
                                     keepdim=True)[0]
    ...
    def masked_softmax(self, scores, mask):
        """
        Used to calculcate a softmax score with true sequence length
        (without padding), rather than max-sequence length.
        Input shape: (batch_size, max_seq_length, hidden_dim).
        mask parameter: Tensor of shape (batch_size, max_seq_length).
        Such a mask is given by the length() function.
        """
